"""Backfill research_backfill_persist3_v1 regime labels.

Writes status='diagnostic', logic_version='research_backfill_persist3_v1'
to context_daily. Idempotent on (as_of_date, context_name, logic_version).
NEVER touches v1 production rows or current B2 logic.

Run:
    DATABASE_URL=postgresql+psycopg://... ./.venv/Scripts/python.exe \
        -m scripts.backfill_research_regime_persist3 [--start 2018-01-02]
"""
...
from apps.api.src.research.regime_backfill_persist3 import (
    LOGIC_VERSION, SOURCE_FEATURES, STATUS, classify_pit, logic_hash,
)
...
def main() -> int:
    ...
    rows: list[dict] = []
    n_stress = 0; n_direct = 0; n_neutral = 0; n_insuf = 0
