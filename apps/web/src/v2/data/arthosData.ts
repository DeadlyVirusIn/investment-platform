...
export const OPPORTUNITIES_TODAY: OpportunityCard[] = [
  // ────────── TIER 1 — STRONGEST SETUPS TODAY ──────────
  {
    symbol: 'TSLA',
    kind: 'stock',
    tier: 'strongest-setups',
    actionLabel: 'Opening a short on TSLA at $172',
    entry: 'Short $172',
    target: '$148',
    invalidate: 'Daily close above $182',
    ...
    setup: {
      score: 4,
      criteria: [
        { label: 'Signal age', met: true, note: 'Fresh — first session of this read', lessonSlug: 'what-is-a-signal' },
        { label: 'Signal density', met: true, note: '3 independent reads agree (inventory, margin, momentum)' },
        { label: 'Liquidity', met: true, note: 'Deep — average daily volume 110M shares' },
        { label: 'Risk-reward', met: true, note: '3:1 — $24 upside vs $10 invalidation' },
        { label: 'Base rate', met: false, note: 'Limited precedent — only 4 prior matches in trailing 3 years' },
      ],
    },
    intelligence: {
      whyItMatters:
        "European port inventory has risen against a flat production schedule. Historically this setup has preceded margin pressure on a one-to-three-month horizon. We're sizing for a multi-week move, not a print event.",
      whatCouldChangeIt:
        "Inventory data reverses by the next reading (mid-week), OR a daily close above $182 invalidates the entry — either ends the trade.",
      whatWeWatch: [
        'Mid-week European port inventory release',
        'Margin commentary at the auto-industry conference (May 28)',
        'Daily close vs $182 invalidation level',
      ],
    },
    lessonSlug: 'what-is-a-signal',
    placeable: true,
  ...
  },
  {
    ...
    setup: {
      score: 5,
      criteria: [
        { label: 'Signal age', met: true, note: 'IV in bottom-quartile for 7 consecutive sessions' },
        { label: 'Signal density', met: true, note: '2 independent reads (IV rank + skew compression)' },
        { label: 'Liquidity', met: true, note: 'Penny-wide bid/ask on both strikes' },
        { label: 'Risk-reward', met: true, note: '0.47:1 risk — defined-risk by construction' },
        { label: 'Base rate', met: true, note: '11 of 14 similar IV-rank setups paid out in trailing year' },
      ],
    },
    intelligence: {
      whyItMatters:
        "Implied volatility on the targeted expiry sits in the bottom quartile of the trailing year. Upside exposure is unusually inexpensive — we're paying $3.40 for what historically costs $4.80 at neutral IV.",
      whatCouldChangeIt:
        "AAPL daily close below $168 invalidates the directional read. Earnings move outside the implied range eliminates the volatility edge.",
      whatWeWatch: [
        'IV expansion before expiry',
        'AAPL daily close vs $168',
        'Underlying price relative to $175 short strike',
      ],
    },
    lessonSlug: 'what-is-a-signal',
    placeable: true,
  ...
  // ────────── TIER 2 — SETUPS STILL FORMING ──────────
  {
    ...
    kind: 'stock',
    tier: 'setups-forming',
    actionLabel: 'Starter long in CRM at $268',
    entry: 'Long $268',
    target: '$298',
    invalidate: 'Daily close below $258',
    sizing: 'Half position',
    setup: {
      score: 3,
      criteria: [
        { label: 'Signal age', met: true, note: '5 sessions of momentum read' },
        { label: 'Signal density', met: false, note: 'Only 1 read firing (cohort momentum); fundamentals neutral' },
        { label: 'Liquidity', met: true, note: 'Deep — 8M shares average' },
        { label: 'Risk-reward', met: true, note: '3:1' },
        { label: 'Base rate', met: false, note: 'Signal too young to score the base rate cleanly' },
      ],
    },
    intelligence: {
      whyItMatters:
        "Software-cohort momentum has turned positive after a six-week consolidation. CRM is one of three names leading the read. We're starting at half size because the signal is younger than our usual entry threshold.",
      whatCouldChangeIt:
        "Two more sessions without confirmation, OR a daily close below $258 — either takes the signal off the board.",
      whatWeWatch: [
        'Software cohort relative strength over next 3 sessions',
        'Whether the signal extends to ORCL and ADBE',
        'June 4 earnings — sizing review beforehand',
      ],
    },
    lessonSlug: 'what-is-a-signal',
    placeable: true,
    ...
    defaultQuantity: 8,
    paragraph:
      "We're opening a half-position long in CRM at $268. Software-cohort momentum has turned positive after a six-week consolidation; CRM leads the read. Half-size for now because the signal is younger than we usually require for full sizing.",
  },

  // ────────── TIER 3 — NAMES WE'RE TRACKING ──────────
  // (Render as a compact list, not full IntelligenceCard)
];
...
// ----------------------------------------------------------------------------
// CATALYSTS — Phase 2 surface
// ----------------------------------------------------------------------------
//
// Catalysts are NOT a calendar. Each catalyst is an Intelligence Card
// answering "why does this matter now?" — the event is the anchor, the
// disposition + why-it-matters is the value.
//
// Disposition vocabulary is the same we use across the product:
//   holding · watching · reducing pre-event · no action · reviewing thesis
//
// Every catalyst surfaces:
//   - event (date + title + kind)
//   - our disposition (plain-language stance)
//   - why it matters now
//   - what could change our view
//   - affected portfolio positions
//   - affected opportunities (Tier 1 / 2 names this catalyst could move)
//   - lesson link
...
export type CatalystDisposition =
  | 'holding'
  | 'watching'
  | 'reducing pre-event'
  | 'no action'
...
export interface CatalystCard {
  id: string;
  date: string;             // ISO
  prettyDate: string;       // 'Tuesday May 27'
  daysFromToday: number;
  horizon: CatalystHorizon;
  kind: CatalystKind;
  kindLabel: string;        // 'Earnings' / 'Macro release' / etc.
  symbol?: string;          // for symbol-specific catalysts
  title: string;            // 'AMAT earnings'

  disposition: CatalystDisposition;
  dispositionDetail: string; // 1-sentence elaboration

  whyItMatters: string;     // 2-4 sentence prose
  whatCouldChangeView: string; // 1-2 sentence prose

  affectedPositions: string[];     // ticker list — held names
  affectedOpportunities: string[]; // ticker list — Opp Tier 1/2/3
  lessonSlug: string;
}

export const CATALYSTS_AHEAD: CatalystCard[] = [
  // ────────── THIS WEEK ──────────
  {
    id: 'unh-review-2026-05-22',
    date: '2026-05-22',
    prettyDate: 'Today',
    daysFromToday: 0,
    horizon: 'this-week',
    kind: 'thesis-review',
    kindLabel: 'Thesis review',
    symbol: 'UNH',
    title: 'UNH — review the half-position',
    disposition: 'reviewing thesis',
    dispositionDetail:
      "We've held this 22 sessions without the conviction we sized for. Today is decision day: full size, half size, or close.",
    whyItMatters:
      'Conviction has a sell-by date. Carrying a position past the point where the thesis can defend it costs more than the position is worth — in attention, in book weight, and in the trade we don\'t take because the cash is committed. Today\'s review forces the answer the original thesis was supposed to provide.',
    whatCouldChangeView:
      "A material data point in the next 12 hours — a guidance pre-announcement or a regulatory headline — would change the review window. Absent that, the review proceeds.",
    affectedPositions: ['UNH'],
    affectedOpportunities: [],
    lessonSlug: 'why-great-investors-do-nothing-most-days',
  },
  {
    id: 'cost-comps-2026-05-29',
    date: '2026-05-29',
    prettyDate: 'Thursday May 29',
    daysFromToday: 7,
    horizon: 'this-week',
    kind: 'corp-action',
    kindLabel: 'Monthly comps',
    symbol: 'COST',
    title: 'COST monthly sales comps',
    disposition: 'holding',
    dispositionDetail:
      "Routine monthly print. We're holding the position; we don't act on a single comp number.",
    whyItMatters:
      "Same-store sales on a multi-year compounder are a trend, not an event. The reading either confirms the model (+5% on flat traffic = pricing power working) or starts to bend it. Either way, it's one print in a longer series — we look at three before we'd update our view.",
    whatCouldChangeView:
      'A negative comp print would be the first since 2024 and would warrant an early thesis review. A flat traffic + flat-pricing readout would compress the moat narrative.',
    affectedPositions: ['COST'],
    affectedOpportunities: [],
    lessonSlug: 'why-great-investors-do-nothing-most-days',
  },
  ...
  // ────────── NEXT WEEK ──────────
  {
    id: 'amat-earnings-2026-05-30',
    date: '2026-05-30',
    prettyDate: 'Friday May 30',
    daysFromToday: 8,
    horizon: 'next-week',
    kind: 'earnings',
    kindLabel: 'Earnings',
    symbol: 'AMAT',
    title: 'AMAT earnings — post-market',
    disposition: 'reducing pre-event',
    dispositionDetail:
      "We trimmed AMAT yesterday because of position-size drift, not because of the earnings event. We will not add into the print.",
    whyItMatters:
      "We held AMAT through six weeks of cohort momentum. The earnings print either confirms the semicap cycle is broader than the headline chip cohort (which is the thesis) or starts to reset that read. Either outcome is a thesis update, not a trade signal. We watch the cohort reaction more than the single name.",
    whatCouldChangeView:
      "Cohort-wide capex commentary at the print — particularly from large customers (Intel, TSMC, Samsung) — would either extend or break the multi-week thesis in a single afternoon. The position is sized for that asymmetry.",
    affectedPositions: ['AMAT'],
    affectedOpportunities: [],
    lessonSlug: 'what-is-a-signal',
  },
  {
    id: 'fomc-minutes-2026-06-02',
    date: '2026-06-02',
    prettyDate: 'Monday June 2',
    daysFromToday: 11,
    horizon: 'next-week',
    kind: 'macro',
    kindLabel: 'Macro release',
    title: 'FOMC minutes release · 2:00pm ET',
    disposition: 'no action',
    dispositionDetail:
      "Macro context only. We don't trade FOMC reactions; we read them to update the regime context the portfolio is built inside.",
    whyItMatters:
      "Minutes are a calibration on what the committee was actually arguing about — vs the press conference framing. The reading either confirms the rate path the portfolio assumes (steady) or hints at a regime shift that affects multiple positions at once (rate-sensitive names, duration exposure). The whole book reads through this lens.",
    whatCouldChangeView:
      "A more hawkish dispersion than the press conference implied would compress our long-duration exposure (MSFT, GOOGL, V) at the margin and could move us to reduce CRM half-size before adding.",
    affectedPositions: ['MSFT', 'GOOGL', 'V', 'JPM'],
    affectedOpportunities: ['CRM'],
    lessonSlug: 'why-prices-move-when-nothing-happens',
  },

  // ────────── TWO WEEKS OUT ──────────
  {
    id: 'cpi-print-2026-06-04',
    date: '2026-06-04',
    prettyDate: 'Wednesday June 4',
    daysFromToday: 13,
    horizon: 'two-weeks-out',
    kind: 'macro',
    kindLabel: 'Macro release',
    title: 'May CPI print · 8:30am ET',
    disposition: 'watching',
    dispositionDetail:
      "We re-read the inflation regime after this print. The portfolio's macro tilt is set by the regime, not the headline.",
    whyItMatters:
      "CPI is the single highest-leverage macro print on a multi-week horizon. A surprise in either direction resets the rate path expectation, which resets equity multiples, which resets every position we hold. We don't act on the print itself — we update the regime read it produces.",
    whatCouldChangeView:
      "A core CPI surprise > 0.15% vs consensus in either direction moves the regime from 'steady' to either 'reflationary' or 'disinflationary'. Each branch changes which lane of opportunities we'd favor.",
    affectedPositions: ['MSFT', 'GOOGL', 'V', 'COST', 'JPM', 'LLY'],
    affectedOpportunities: ['TSLA', 'CRM'],
    lessonSlug: 'why-prices-move-when-nothing-happens',
  },
  {
    id: 'aapl-spread-expiry-2026-06-06',
    date: '2026-06-06',
    prettyDate: 'Friday June 6',
    daysFromToday: 15,
    horizon: 'two-weeks-out',
    kind: 'corp-action',
    kindLabel: 'Options expiry',
    symbol: 'AAPL',
    title: 'AAPL 175/180 call spread expiry',
    disposition: 'watching',
    dispositionDetail:
      "If we placed the Tier-1 options idea this week, this is its expiration. The defined-risk structure means our outcome is bounded; the value here is calibration data.",
    whyItMatters:
      "The structure was a low-IV entry — historically those have paid out 11 of 14 times in our trailing year. The actual outcome will be one more data point in that base rate, either confirming or compressing it. The trade itself is small; the lesson it provides is not.",
    whatCouldChangeView:
      "An outcome materially worse than expected at our entry IV would lower the base rate confidence for the next setup of this type.",
    affectedPositions: [],
    affectedOpportunities: ['AAPL'],
    lessonSlug: 'what-is-a-signal',
  },

  // ────────── BEYOND ──────────
  {
    id: 'semicap-data-2026-06-12',
    date: '2026-06-12',
    prettyDate: 'Wednesday June 12',
    daysFromToday: 21,
    horizon: 'beyond',
    kind: 'industry',
    kindLabel: 'Industry data',
    title: 'Semicap industry sales data',
    disposition: 'watching',
    dispositionDetail:
      "Cohort-wide readout. More important than any single name's print because it tells us whether the multi-week momentum thesis still holds across the group.",
    whyItMatters:
      "Single-stock prints can be noisy. Industry-wide sales data is the cleanest read on whether the semicap cycle we sized AMAT for is broadening or narrowing. If the cohort is broadening, AMAT stays at current weight; if it's narrowing, we reduce further.",
    whatCouldChangeView:
      "A sequentially negative cohort number (-3% MoM or worse) would compress the multi-week thesis and likely take AMAT to a third trim.",
    affectedPositions: ['AMAT'],
    affectedOpportunities: [],
    lessonSlug: 'what-is-a-signal',
  },
  {
    id: 'msft-earnings-2026-06-25',
    date: '2026-07-25',
    prettyDate: 'Friday July 25',
    daysFromToday: 64,
    horizon: 'beyond',
    kind: 'earnings',
    kindLabel: 'Earnings',
    symbol: 'MSFT',
    title: 'MSFT earnings — post-market',
    disposition: 'holding',
    dispositionDetail:
      "Multi-quarter thesis position. The print either confirms the cash-flow gap we opened it on, or starts to reset it. We hold into it.",
    whyItMatters:
      "Twelve months of operating-cash-flow growth above the software cohort sits at the center of our thesis here. Cloud-bookings line will either extend that pattern or be the first quarter of compression. The first signal of compression is what we'd respect; one print does not break a multi-quarter case alone.",
    whatCouldChangeView:
      "A guidance reset on cloud bookings + a margin step-down in the same print would be the first material thesis bend since entry.",
    affectedPositions: ['MSFT'],
    affectedOpportunities: [],
    lessonSlug: 'why-prices-move-when-nothing-happens',
  },
];
...
// ----------------------------------------------------------------------------
//
// Curated daily intelligence. NOT a news feed. Lens-grouped:
//   1. Material to what we hold        (portfolio)
//   2. Material to what we're considering  (considering — Opps + Watchlist)
//   3. Material to the regime           (regime — macro / cohort)
//
// Hard cap: 6 visible per day. Source ceiling: 12 stored. Curated-from
// counter ("curated 6 from 23 candidates") is the trust mechanism that
// keeps this from becoming a news feed.

export type FieldNoteReading =
  | 'confirming thesis'
  | 'breaking thesis'
  | 'no change'
  | 'watching'
...
export type FieldNoteImpact = 'portfolio' | 'considering' | 'regime';

export interface FieldNote {
  id: string;
  postedAt: string;             // ISO
  prettyTime: string;           // '4:12pm ET'
  kind: FieldNoteKind;
  kindLabel: string;            // 'Earnings' / 'Filing' / etc.
  symbol?: string;
  headline: string;             // ArthOS-lens phrasing
  whatHappened: string;
  whyItMatters: string;
  whatWeObserved: string;
  reading: FieldNoteReading;
  readingNote: string;
  whatCouldChange: string;
  affectedPositions: string[];
  affectedOpportunities: string[];
  firedCatalystId?: string;
  impact: FieldNoteImpact;
  lessonSlug: string;
  sources: string[];
}

export const FIELD_NOTES_TODAY: FieldNote[] = [
  // ───── PORTFOLIO ─────
  {
    id: 'fn-msft-earnings-2026-05-22',
    postedAt: '2026-05-22T16:12:00-04:00',
    prettyTime: '4:12pm ET',
    kind: 'earnings',
    kindLabel: 'Earnings',
    symbol: 'MSFT',
    headline: 'MSFT cash-flow line confirms the thesis we hold it for.',
    whatHappened:
      'MSFT reported Q4 OCF growth of +13.8% YoY — 70bps above the cohort. Seven consecutive quarters of relative outperformance now in the record.',
    whyItMatters:
      'This is the cash-flow gap we sized the position for at entry. The 7-quarter pattern was the load-bearing claim in the original thesis review. Today\'s print extends it without complication.',
    whatWeObserved:
      'Cloud bookings line +21% vs +18% consensus. Gross margin held within a 30bp band. The multiple-expansion trigger we wrote about in the May 1 thesis review remains valid.',
    reading: 'confirming thesis',
    readingNote:
      'Holding the position at current size. No re-read of the thesis is required this cycle.',
    whatCouldChange:
      'A material reversal in cloud bookings growth (<+12%) at the next print, or a sequential margin step-down, would warrant an earlier thesis review.',
    affectedPositions: ['MSFT'],
    affectedOpportunities: [],
    firedCatalystId: 'msft-earnings-2026-06-25',
    impact: 'portfolio',
    lessonSlug: 'why-prices-move-when-nothing-happens',
    sources: ['Company filings', 'Tiingo fundamentals'],
  },
  {
    id: 'fn-amat-trim-2026-05-22',
    postedAt: '2026-05-22T15:48:00-04:00',
    prettyTime: '3:48pm ET',
    kind: 'corp-action',
    kindLabel: 'Position action',
    symbol: 'AMAT',
    headline: 'AMAT trim closed at $199.50 — position now at sized weight.',
    whatHappened:
      'We sold a quarter of the AMAT position. The trim was a response to position-size drift (5.2% → 4.0% of book), not to any change in the underlying thesis.',
    whyItMatters:
      'Position size discipline is the load-bearing tool we have for managing winners. The thesis is intact; the weight is now back to where we sized it at entry. The remaining 3/4 of the position runs on the original target.',
    whatWeObserved:
      'Average fill price was 4bps better than the morning indication. Cash returned: $7,180. Will be redeployed when the next signal warrants it — not before.',
    reading: 'reducing',
    readingNote:
      'Reducing size, not exiting. The semicap-cohort momentum signal that opened this trade is still active.',
    whatCouldChange:
      'A close below $192 on AMAT would invalidate the momentum signal and trigger a full exit. Earnings May 30 — we will not add into the print.',
    affectedPositions: ['AMAT'],
    affectedOpportunities: [],
    impact: 'portfolio',
    lessonSlug: 'why-great-investors-do-nothing-most-days',
    sources: ['Internal portfolio accounting'],
  },

  // ───── CONSIDERING ─────
  {
    id: 'fn-nflx-analyst-2026-05-22',
    postedAt: '2026-05-22T11:30:00-04:00',
    prettyTime: '11:30am ET',
    kind: 'analyst',
    kindLabel: 'Analyst revision',
    symbol: 'NFLX',
    headline: 'NFLX consensus target lifted; the read matches our cohort signal.',
    whatHappened:
      'Three sell-side analysts raised their NFLX targets in the last 48 hours, pushing the consensus median 9% higher. None of the upgrades cite the cohort-momentum read we use; the alignment is coincidental.',
    whyItMatters:
      'NFLX is on the Watchlist. We were tracking the early formation of a momentum read; the analyst revisions are a second, independent read pointing the same direction. We do not upgrade the Setup Strength on analyst flow alone — but the convergence does sharpen the signal.',
    whatWeObserved:
      'Average target now $710 (vs $651 a week ago). Two-of-three upgrades cited subscriber-trajectory inflection — a read we cannot independently verify, so we keep it as context not confirmation.',
    reading: 'watching',
    readingNote:
      'Still tracking. The cohort read needs two more sessions of confirmation before we would consider a starter position.',
    whatCouldChange:
      'Confirmation from the cohort momentum read over the next two sessions would move NFLX from Tier 3 (tracking) to Tier 2 (forming).',
    affectedPositions: [],
    affectedOpportunities: ['NFLX'],
    impact: 'considering',
    lessonSlug: 'what-is-a-signal',
    sources: ['Sell-side consensus aggregator', 'Tiingo daily bars'],
  },
  {
    id: 'fn-aapl-iv-2026-05-22',
    postedAt: '2026-05-22T09:42:00-04:00',
    prettyTime: '9:42am ET',
    kind: 'corp-action',
    kindLabel: 'Vol regime',
    symbol: 'AAPL',
    headline: 'AAPL implied vol fell into the year\'s bottom-decile this morning.',
    whatHappened:
      'AAPL 30-day implied vol opened at the lowest level in the trailing year — below the entry threshold we sized the Tier-1 call spread for. The structure now buys roughly the same exposure for $0.30 less than yesterday.',
    whyItMatters:
      'The Tier-1 options idea on the Opportunities page was constructed at $3.40 debit. Today\'s IV print would put the same structure closer to $3.10. The signal that put AAPL on the desk is now stronger, not weaker.',
    whatWeObserved:
      'IV rank dropped to 8th percentile. Skew unchanged. Bid/ask still penny-wide on both strikes — liquidity is intact.',
    reading: 'confirming thesis',
    readingNote:
      'The setup is now more attractive than at the time of the original proposal. Same disposition as yesterday: place at full size.',
    whatCouldChange:
      'A daily close on AAPL below $168 invalidates the directional read regardless of the IV environment.',
    affectedPositions: [],
    affectedOpportunities: ['AAPL'],
    impact: 'considering',
    lessonSlug: 'what-is-a-signal',
    sources: ['Tradier options chain', 'ThetaData IV history'],
  },

  // ───── REGIME ─────
  {
    id: 'fn-cpi-dispersion-2026-05-22',
    postedAt: '2026-05-22T08:35:00-04:00',
    prettyTime: '8:35am ET',
    kind: 'macro',
    kindLabel: 'Macro release',
    headline: 'Pre-CPI dispersion narrowed overnight — markets agree more than usual.',
    whatHappened:
      'The interquartile range across sell-side core-CPI forecasts compressed to 3bps overnight from 7bps last week. The range is the narrowest in five months.',
    whyItMatters:
      'A narrower forecast distribution does not change our expected outcome — but it does change what a surprise would mean. A narrower distribution makes the same magnitude surprise more market-moving on the day. The portfolio carries duration; this affects how we read the print on June 4.',
    whatWeObserved:
      'The compression is concentrated in shelter (services side); goods inflation dispersion unchanged. The range narrowed without a meaningful change in the median.',
    reading: 'watching',
    readingNote:
      'No portfolio action. We update how we read the June 4 print against this calibration, not how we positioned for it.',
    whatCouldChange:
      'A wider re-dispersion before the print would re-flatten the reaction function. Watch the next two sessions for that.',
    affectedPositions: ['MSFT', 'GOOGL', 'V', 'COST', 'JPM', 'LLY'],
    affectedOpportunities: ['TSLA', 'CRM'],
    firedCatalystId: 'cpi-print-2026-06-04',
    impact: 'regime',
    lessonSlug: 'why-prices-move-when-nothing-happens',
    sources: ['Sell-side forecast aggregator', 'FRED release calendar'],
  },
  {
    id: 'fn-semicap-cohort-2026-05-22',
    postedAt: '2026-05-22T07:50:00-04:00',
    prettyTime: '7:50am ET',
    kind: 'macro',
    kindLabel: 'Cohort data',
    headline: 'Semicap cohort momentum extended to a seventh consecutive week.',
    whatHappened:
      'The semiconductor capital-equipment cohort closed last week +1.4%, the seventh consecutive week of outperformance vs the broader chip index. The read is the longest single-cohort streak we track since 2022.',
    whyItMatters:
      'AMAT is a held position sized for this exact cohort thesis. The streak extension is not new information — it is continuation of the read the position was opened on. The interesting datapoint is the duration: streaks past six weeks have historically continued another four weeks 71% of the time.',
    whatWeObserved:
      'The cohort outperformance was broad-based — 5 of 7 cohort members positive. AMAT was 4th-best in the cohort week-over-week.',
    reading: 'no change',
    readingNote:
      'The cohort thesis remains intact. We trimmed AMAT today for size, not for read.',
    whatCouldChange:
      'A cohort-wide negative week (3+ of 7 names red) would compress the multi-week thesis and likely take AMAT to a third trim.',
    affectedPositions: ['AMAT'],
    affectedOpportunities: [],
    firedCatalystId: 'semicap-data-2026-06-12',
    impact: 'regime',
    lessonSlug: 'what-is-a-signal',
    sources: ['Cohort relative-strength scan', 'Tiingo daily bars'],
  },
];

export const FIELD_NOTES_CURATED_FROM = 23;

export function getFieldNotesByImpact(
  impact: FieldNoteImpact
): FieldNote[] {
  return FIELD_NOTES_TODAY.filter((n) => n.impact === impact);
}

export function getFieldNotesForSymbol(symbol: string): FieldNote[] {
  const sym = symbol.toUpperCase();
  return FIELD_NOTES_TODAY.filter(
    (n) =>
      n.symbol === sym ||
      n.affectedPositions.includes(sym) ||
      n.affectedOpportunities.includes(sym)
  );
}

export function getFieldNoteForCatalyst(
  catalystId: string
): FieldNote | undefined {
  return FIELD_NOTES_TODAY.find((n) => n.firedCatalystId === catalystId);
}

// ----------------------------------------------------------------------------
// MARKET PULSE — Phase 4
// ----------------------------------------------------------------------------
//
// Answers "what kind of market is this?" — qualitative regime read
// first, quantitative anchor second. No raw % changes leading; no
// red/green; no alerts. Same shape backend will produce once
// /api/market/pulse extends from /api/market/tape with VIX + sector
// ETFs + breadth + US10Y.

export type PulseTone = 'calm' | 'neutral' | 'tilted';

export interface PulseTile {
  label: string;          // 'Regime'
  value: string;          // 'Steady'           ← qualitative-first
  context: string;        // '11 sessions inside the same band'
  anchor?: string;        // 'VIX 13.82'        ← quantitative anchor
  tone: PulseTone;        // visual weight only, never red/green
}

export interface MarketPulse {
  asOf: AsOfStamp;
  narrative: string;      // 2-3 sentence character read of the day
  tiles: PulseTile[];     // 6 tiles
  source: string;         // 'Polygon delayed quotes · 15-min delay'
}

export const MARKET_PULSE_TODAY: MarketPulse = {
  asOf: BRIEFING_AS_OF,
  narrative:
    "A quiet, broad-participation Friday. Volatility is sitting in the lowest week of the month. Rates stayed inside their three-week band. Nine of eleven sectors closed green. There is nothing in today's tape that asks for a portfolio change — this is the kind of market where doing nothing is the work.",
  tiles: [
    {
      label: 'Regime',
      value: 'Steady',
      context: '11 sessions inside the same band',
      anchor: 'no rotation visible',
      tone: 'calm',
    },
    {
      label: 'Volatility',
      value: 'Calm',
      context: 'Lowest week of the month',
      anchor: 'VIX 13.82',
      tone: 'calm',
    },
    {
      label: 'Breadth',
      value: 'Even',
      context: 'No leadership tilt today',
      anchor: '52% advancing',
      tone: 'neutral',
    },
    {
      label: 'Participation',
      value: 'Broad',
      context: 'Most sectors green by small amounts',
      anchor: '9 of 11 sectors',
      tone: 'calm',
    },
    {
      label: 'Trend',
      value: 'Holding range',
      context: 'S&P inside a 6-day band',
      anchor: '±0.5% week',
      tone: 'neutral',
    },
    {
      label: 'Rates',
      value: 'Anchored',
      context: '10-year unchanged from last reading',
      anchor: 'US10Y 4.29%',
      tone: 'calm',
    },
  ],
  source: 'Polygon delayed quotes · 15-min delay',
};

// ────────── Personalization helpers ──────────
// Pure functions over the static data. Watchlist is the only input
// that varies per user; the rest is shared. Phase 3 surfaces intersect
// watchlist with each of these to render the personalized panels.

export interface SymbolContext {
  symbol: string;
  mentionedInChangedToday: boolean;
  changedToday?: ChangedItem;
  opportunityTier: OpportunityTier | null;
  opportunity?: OpportunityCard;
  tracking?: TrackingName;
  catalystsThisHorizon: CatalystCard[];
  catalystsAffecting: CatalystCard[];   // events where symbol is in affectedPositions OR affectedOpportunities
  held: boolean;
  position?: Position;
}
...
export function getSymbolContext(symbol: string): SymbolContext {
  const sym = symbol.toUpperCase();
  const changedToday = WHAT_CHANGED_SINCE_YESTERDAY.find(
    (c) => c.symbol === sym
  );
  const opportunity = OPPORTUNITIES_TODAY.find((o) => o.symbol === sym);
  const tracking = TRACKING_NAMES.find((t) => t.symbol === sym);
  const opportunityTier: OpportunityTier | null = opportunity
    ? opportunity.tier
    : tracking
      ? 'tracking'
      : null;
  const catalystsAffecting = CATALYSTS_AHEAD.filter(
    (c) =>
      c.symbol === sym ||
      c.affectedPositions.includes(sym) ||
      c.affectedOpportunities.includes(sym)
  );
  const catalystsThisHorizon = catalystsAffecting.filter(
    (c) => c.horizon === 'this-week' || c.horizon === 'next-week'
  );
  const position = PORTFOLIO.positions.find((p) => p.symbol === sym);
  return {
    symbol: sym,
    mentionedInChangedToday: !!changedToday,
    changedToday,
    opportunityTier,
    opportunity,
    tracking,
    catalystsThisHorizon,
    catalystsAffecting,
    held: !!position,
    position,
  };
...
export const HORIZON_LABELS: Record<CatalystHorizon, string> = {
  'this-week': 'This week',
  'next-week': 'Next week',
